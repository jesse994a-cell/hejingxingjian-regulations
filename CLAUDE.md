...
│   └── ...（詳見 index.html 公共事務面板）
│
├── SOP/                                                  ← SOP 作業程序目錄（PDF，納入版控）
│   ├── 20251113_和境心見保全執勤SOP.pdf
│   ├── 20260101_社區安全維護SOP作業程序.pdf
│   └── 20260205_住戶家無水緊急情況SOP.pdf
│
...
1. **最後更新日期**：每次修改任何 `.md` 文件內容後，必須同步將文件頂部的 `**最後更新：**` 欄位（若有）或 `**制定日期：**` 後方備註更新為**修改當天日期**（格式 `YYYY.MM.DD`）。
2. **版本號一致**：`.md` 文件內部的 `**版本：**` 欄位，必須與 `index.html` 的 `docs` 陣列中對應的 `version` 欄位**保持完全相同**。
   - 修改 MD 內容時：同步升版（如 v0.1 → v0.2），並同步更新 `index.html` 的 `version` 欄位。
   - 禁止只改一邊而忽略另一邊。
3. **版本升版原則**：
   - 小幅修正（文字錯誤、用語調整）：第三位數字 +1（如 v0.1 → v0.1，視範圍而定）
   - 章節新增或結構調整：次版本 +1（如 v0.1 → v0.2）
   - 正式通過後改為 v1.0
